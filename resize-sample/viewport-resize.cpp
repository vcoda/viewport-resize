...
class ViewportResizeApp : public VulkanApp
{
...
public:
    ViewportResizeApp(const AppEntry& entry):
        VulkanApp(entry, TEXT("Viewport resize sample"), 512, 512)
    {
        initialize();
        setupView();
        createVertexBuffer();
    ...
    void setupPipeline()
    {
        ...
        graphicsPipeline = std::make_unique<GraphicsPipeline>(device,
            ...
            magma::renderstate::fillCullNoneCcw,
...
{
    return std::unique_ptr<ViewportResizeApp>(new ViewportResizeApp(entry));
}
